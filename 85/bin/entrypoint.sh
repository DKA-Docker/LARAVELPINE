...
if [ -f /etc/php85/php.ini ]; then
  if [ -n "$DKA_PHP_MEMORY_LIMIT" ] || [ -n "$DKA_PHP_UPLOAD_MAX_FILESIZE" ] || [ -n "$DKA_PHP_POST_MAX_SIZE" ] || [ -n "$DKA_PHP_MAX_EXECUTION_TIME" ] || [ -n "$DKA_PHP_MAX_INPUT_VARS" ]; then
    ...
    [ -n "$DKA_PHP_MEMORY_LIMIT" ] && sed -i "s/^\s*memory_limit\s*=.*/memory_limit = ${DKA_PHP_MEMORY_LIMIT}/" /etc/php85/php.ini
    [ -n "$DKA_PHP_UPLOAD_MAX_FILESIZE" ] && sed -i "s/^\s*upload_max_filesize\s*=.*/upload_max_filesize = ${DKA_PHP_UPLOAD_MAX_FILESIZE}/" /etc/php85/php.ini
    [ -n "$DKA_PHP_POST_MAX_SIZE" ] && sed -i "s/^\s*post_max_size\s*=.*/post_max_size = ${DKA_PHP_POST_MAX_SIZE}/" /etc/php85/php.ini
    [ -n "$DKA_PHP_MAX_EXECUTION_TIME" ] && sed -i "s/^\s*max_execution_time\s*=.*/max_execution_time = ${DKA_PHP_MAX_EXECUTION_TIME}/" /etc/php85/php.ini
    [ -n "$DKA_PHP_MAX_INPUT_VARS" ] && sed -i "s/^;*\s*max_input_vars\s*=.*/max_input_vars = ${DKA_PHP_MAX_INPUT_VARS}/" /etc/php85/php.ini
  fi
fi
